...
function validateCreateInput(input) {
  ...
  if (input.source !== undefined && input.source !== null) {
    ...
    if (!new Set(["slash", "mention"]).has(input.source.kind))
...
export class JobStore {
  ...
  create({ requestId, requesterDiscordId, requesterName, objective, acceptanceCriteria = [], scope = [], source = null }) {
    validateCreateInput({ requestId, requesterDiscordId, requesterName, objective, acceptanceCriteria, scope, source });
    const timestamp = now();
    ...
    const statement = this.db.prepare(`
      INSERT INTO jobs (
        ...
        source_channel_id, source_message_id, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, 'provisioning', ?, ?, ?, ?, ?, ?)
    `);
  ...
  claimProvisioning(id, claimToken, leaseSeconds = 120) {
    ...
    const result = this.db.prepare(`
      ...
      WHERE id = ? AND state = 'provisioning' AND discord_thread_id IS NULL
  ...
  listRecoverableProvisioning(staleSeconds = 120, limit = 10) {
    ...
    return this.db.prepare(`
      SELECT * FROM jobs
      WHERE state = 'provisioning' AND source_kind = 'mention' AND discord_thread_id IS NULL
        AND (provisioning_claim IS NULL OR provisioning_claimed_at < ?)
      ORDER BY created_at
      LIMIT ?
  ...
  claim(runnerId, leaseSeconds = 300) {
    ...
    try {
      ...
      const row = this.db.prepare("SELECT id FROM jobs WHERE state = 'queued' ORDER BY created_at LIMIT 1").get();
      if (!row) {
        this.db.exec("COMMIT");
        return null;
      }
