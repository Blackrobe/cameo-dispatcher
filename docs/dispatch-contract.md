...
- Natural task-thread wording cannot authorize a GitHub mutation. A possible single merge/close target creates only a signed, 15-minute Discord proposal showing the freshly fetched upstream PR, head/base branches, and head SHA while ordinary agent analysis continues. A click from either trusted developer is the distinct authorization interaction. The server binds it to the proposal message, task thread, PR, and displayed SHA; the Windows controller independently resolves and journals the full identity and rejects drift before mutation. `/cameo-github` remains the explicit structured fast path. Merge requests an immediate REST merge at that SHA; unmet checks, reviews, protection, or queue requirements fail closed and never enable auto-merge. No admin bypass, direct target-branch push, force-push, branch deletion, or unsolicited comment is permitted.
